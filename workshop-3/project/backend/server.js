...
app.post('/ducks', (req, res) => {
    ...
    if (!image) {
        return res.status(400).json({ error: 'Missing duck image' });
    }

  const stmt = db.prepare('INSERT INTO ducks (name, species, age, image) VALUES (?, ?, ?, ?)');
  stmt.run([name, species, age, image], function (err) {
    if (err) return res.status(500).json({ error: err.message });
